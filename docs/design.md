...
```
     y=0    ~~~~~~~~~~~~~~~~     地平線（画面より 20px 上）に近い。文字は潰れて消える
                --------
                 ------
   y=191   ================     手前。文字が一番大きい
```
...
```c
d      = y - H0                  // その行から地平線までの距離
PA     = CAM_H / d               // 1 画面ピクセルあたりのテクスチャ幅
texRow = BASE + scroll - K / d   // その行が映すテクスチャの行（K = CAM_H * FOCAL）
```

`d` が小さい（画面の上）ほど `PA` が大きく＝文字が小さくなる。`scroll` を増やすと
`texRow` が全体に増え、文字が奥へ吸い込まれていく。
...
| 変えたいこと | 定数 |
| --- | --- |
| 手前の文字の大きさ | `CAM_H`（大きくすると小さくなる） |
| 遠近の強さ・一度に見える行数 | `FOCAL` |
| 地平線の位置 | `H0` |
| 文字が消え始める / 消えきる高さ | `FADE_HI` / `FADE_LO` |
| 流れる速さ | `SPEED`（8.8 固定小数） |
| 本文 | `crawlText[]`。1 行 22 文字まで（手前で画面幅に収まる幅） |

本文は 1 行 3 タイル（24px）、空行 2 タイル（16px）で積む。全部で 64 タイル行に
収める必要があるので、行を増やすときは `CRAWL_END` も合わせて延ばすこと。
